...
Responses.consultas(data,'http://127.0.0.1:8000/api/modificarpractico').then(response=>{
...
if (response.practico!==undefined){
    ...
    if (response.fechavencimiento!==undefined){
       setErrors({...errors,fechavencimiento : response.fechavencimiento});
      ...
      ...form, [e.target.name]: e.target.value
                       ...
                       let regex = /^[0-9a-zA-Z\s]+$/;
                                    if(!form.practico.trim()){
                                        ...
                                        setErrors({...errors,     [e.target.name]: "El campo 'Práctico' es requerido."});
                                    setP('2');
                                    }
                                    
                                    else if(!regex.test(form.practico.trim())){
                                        setErrors({...errors,  [e.target.name] : "El práctico ingresado tiene un formato incorrecto."});
                                        setP('2');
                                    }
                                    else{
                                        var data = {
                                    ...
                                    Responses.consultas(data,'http://127.0.0.1:8000/api/consultarnompractico').then(response=>{
...
<Calendario Cambio={CambioFV} label={"Fecha De Vencimiento"} vpd={FormatoFecha(props.practico.FechaVencimiento)}/>
            ...
            {errors.fechavencimiento}
                        ...
                        <FormControl 
                            ...
                            <InputLabel htmlFor="notaminima">Nota mínima de aprobación</InputLabel>
                            <Input 
                                id="notaminima" 
                                type="text"  
                                name="notaminima"
      ...
      ...form, [e.target.name]: e.target.value
                       ...
                       let regex = /^[0-9]+$/;
                                    if(!form.notaminima.trim()){
                                        ...
                                        setErrors({...errors,     [e.target.name]: "El campo 'Nota mínima de aprobación' es requerido."});
                                    setNM('2');
                                    }
                                    
                                    
                                    else if(!regex.test(form.notaminima.trim())){
                                        setErrors({...errors,  [e.target.name] : "La nota mínima ingresada tiene un formato incorrecto."});
                                        setNM('2');
                                    }

                                    else if(parseInt(form.notaminima)<1){
                                        setErrors({...errors,  [e.target.name] : "La nota mínima ingresada debe ser mayor que 0."});
                                        setNM('2');
                                    }

                                    else if(parseInt(form.notaminima)>parseInt(props.cursada.EscalaPracticos)){
                                        setErrors({...errors,  [e.target.name] : "La nota mínima ingresada debe ser menor que "+ props.cursada.EscalaPracticos});
                                        setNM('2');
