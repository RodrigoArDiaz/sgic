...
export const OrdenarLista = (props) => {
  ...
  function Orden(dato1, dato2){
    ...
    Responses.consultas(data,'http://127.0.0.1:8000/api/ordenarpracticos').then(response=>{
  ...
  {(provided) => (
              <div {...provided.droppableProps} ref={provided.innerRef}>
                ...
                {ordenes.map((row, index) => {
                  ...
                  return (
                    <Draggable key={row.IdPractico.toString()} draggableId={row.IdPractico.toString()} index={index}>
                      {(provided) => (
