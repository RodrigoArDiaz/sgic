...
export const BotonEstado = (props) => {
  ...
  const navegar = useNavigate();
    ...
    if (salto ==='A'){
      setSalto('C');
      Responses.consultas(data,'http://127.0.0.1:8000/api/bajapractico').then(response=>{
    ...
    if (salto ==='B'){
      setSalto('C');
      Responses.consultas(data,'http://127.0.0.1:8000/api/altapractico').then(response=>{
